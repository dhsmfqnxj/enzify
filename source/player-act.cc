...
random_var player::attack_delay_with(const item_def *weap, bool melee_only) const
{
    ...
    else if (is_melee_weapon(*weap) || ranged_weapon_attack)
    {
        ...
        const int wpn_sklev = you.skill(wpn_skill, 10);
        const int min_delay_skill = 10 * weapon_min_delay_skill(*weap);

        attk_delay = random_var(property(*weap, PWPN_SPEED));
        if (is_unrandom_artefact(*weap, UNRAND_WOODCUTTERS_AXE))
            return attk_delay;

        attk_delay = weapon_skill_delay(property(*weap, PWPN_SPEED),
                                        wpn_sklev, min_delay_skill,
